...
public class CoverView extends View {
    ...
    private float focusWidth = 150;
    private float focusHeight = 150;
    private float fozusRadius = 150;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (clickX != 0 && clickY != 0) {
            mPaint.setColor(Color.RED);
            canvas.drawCircle(clickX, clickY, fozusRadius, mPaint);   // 绘制一个小圆形
        }
        if (faces == null) {
            return;
        }
        ...
        if (clickX != 0 && clickY != 0) {  // 这个指的是 寻找到人脸后，画一个蓝色框
...
//            focusArea.bottom = (int) (clickY + focusHeight / 2 > getBottom() ? getBottom() : clickY + focusHeight / 2);
            ...
            Log.d("MyTest","focusArea.left: "+focusArea.left +"   focusArea.top: "+focusArea.top+"  focusArea.right: "+focusArea.right+"  focusArea.bottom: "+focusArea.bottom);
            if (listener != null) {
